...
let rec eval_exp = function
  ...
  | S.Nil -> S.Nil
  | S.Plus (e1, e2) -> S.Int (eval_int e1 + eval_int e2)
  | S.Minus (e1, e2) -> S.Int (eval_int e1 - eval_int e2)
  | S.Times (e1, e2) -> S.Int (eval_int e1 * eval_int e2)
  | S.Equal (e1, e2) -> S.Bool (eval_int e1 = eval_int e2)
  | S.Less (e1, e2) -> S.Bool (eval_int e1 < eval_int e2)
  | S.Greater (e1, e2) -> S.Bool (eval_int e1 > eval_int e2)
  ...
  | S.Apply (e1, e2) ->
      ...
      in
      begin match f with
      | S.Lambda (x, e) -> eval_exp (S.subst [(x, eval_exp e2)] e)
      | S.RecLambda (f, x, e) as rec_f -> eval_exp (S.subst [(f, rec_f); (x, eval_exp e2)] e)
  ...
  | S.Cons (e1, e2) ->
    ...
    | S.Nil -> eval_exp e2
    | S.Int n -> S.Cons (S.Int n, eval_exp e2) 
    | S.Cons (x, xs) -> S.Cons (eval_exp x, S.Cons (eval_exp xs, eval_exp e2))
...
let rec step = function
  ...
  | S.Pair (e1, e2) -> S.Pair (step e1, step e2)
  | S.Fst (S.Pair (e1, e2)) -> step e1
  | S.Snd (S.Pair (e1, e2)) -> step e2
  | S.Cons (S.Int n1, S.Int n2) -> S.Cons (S.Int n1, S.Int n2)
  | S.Cons (S.Int n1, e2) -> S.Cons (S.Int n1, step e2)
  | S.Cons (e1, e2) -> S.Cons (step e1, e2)
  | S.Match (S.Nil, e1, x, xs, e2) -> step e1
  | S.Match (S.Cons (v1, v2), e1, x, xs, e2) -> step e2
